  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="88px" size="small">
      ...
      <el-form-item label="节点名称" prop="goodsStatusName">
        <el-input v-model="queryParams.data.goodsStatusName" clearable placeholder="请输入节点名称"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="预计制造日期" prop="forecastMakeDate">
        <el-input v-model="queryParams.data.forecastMakeDate" clearable placeholder="请输入预计制造日期"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="apsOrderGoodsForecastMakeList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="节点名称" prop="goodsStatusName">
          <el-input v-model="form.goodsStatusName" clearable placeholder="请输入节点名称"/>
        </el-form-item>
        <el-form-item label="预计制造日期" prop="forecastMakeDate">
          <el-input v-model="form.forecastMakeDate" clearable placeholder="请输入预计制造日期"/>
        </el-form-item>
        <el-form-item label="工厂ID" prop="factoryId">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        goodsStatusName: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        forecastMakeDate: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
  ...
  created() {
    document['pagePath'] = '/apsOrderGoodsForecastMake'
    this.getList()
  },
  methods: {
    /** 查询公告列表 */
    getList() {
      ...
      queryPageList(this.queryParams).then(response => {
        ...
        this.apsOrderGoodsForecastMakeList = response.dataList
