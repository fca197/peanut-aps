<template>
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="88px" size="small">
      <el-form-item label="${column.comment}" prop="roomCode">
        <el-input v-model="queryParams.data.roomCode" clearable placeholder="请输入${column.comment}"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="${column.comment}" prop="roomName">
        <el-input v-model="queryParams.data.roomName" clearable placeholder="请输入${column.comment}"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="apsRoomList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="${column.comment}" prop="roomCode">
          <el-input v-model="form.roomCode" clearable placeholder="请输入${column.comment}"/>
        </el-form-item>
        <el-form-item label="${column.comment}" prop="roomName">
          <el-input v-model="form.roomName" clearable placeholder="请输入${column.comment}"/>
        </el-form-item>
        <el-form-item label="工厂ID" prop="factoryId">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 表单校验
      rules: {
        roomCode: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
  ...
  methods: {
    ...
    getList() {
      ...
      queryPageList(this.queryParams).then(response => {
        ...
        this.apsRoomList = response.dataList
