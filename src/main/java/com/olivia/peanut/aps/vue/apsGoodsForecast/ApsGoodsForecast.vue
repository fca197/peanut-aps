  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="88px" size="small">
      ...
      <el-form-item label="预测编码" prop="forecastNo">
        <el-input v-model="queryParams.data.forecastNo" clearable placeholder="请输入预测编码"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="预测名称" prop="forecastName">
        <el-input v-model="queryParams.data.forecastName" clearable placeholder="请输入预测名称"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="开始时间" prop="forecastBeginDate">
        <el-input v-model="queryParams.data.forecastBeginDate" clearable placeholder="请输入开始时间"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="结束时间" prop="forecastEndDate">
        <el-input v-model="queryParams.data.forecastEndDate" clearable placeholder="请输入结束时间"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="apsGoodsForecastList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="开始时间" prop="forecastBeginDate">
          <el-input v-model="form.forecastBeginDate" clearable placeholder="请输入开始时间"/>
        </el-form-item>
        <el-form-item label="结束时间" prop="forecastEndDate">
          <el-input v-model="form.forecastEndDate" clearable placeholder="请输入结束时间"/>
        </el-form-item>
        <el-form-item label="${column.comment}" prop="month">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        forecastNo: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        forecastName: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        forecastBeginDate: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        forecastEndDate: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
  ...
  methods: {
    ...
    getList() {
      ...
      queryPageList(this.queryParams).then(response => {
        ...
        this.apsGoodsForecastList = response.dataList
