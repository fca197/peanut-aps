  ...
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :inline="true" :model="queryParams" label-width="88px" size="small">
      <el-form-item label="物流路径编码" prop="logisticsPathCode">
        <el-input v-model="queryParams.data.logisticsPathCode" clearable placeholder="请输入物流路径编码"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="物流路径名称" prop="logisticsPathName">
        <el-input v-model="queryParams.data.logisticsPathName" clearable placeholder="请输入物流路径名称"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item label="备注" prop="logisticsPathRemark">
        <el-input v-model="queryParams.data.logisticsPathRemark" clearable placeholder="请输入备注"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 表单校验
      rules: {
        logisticsPathCode: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        logisticsPathName: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
        }],
        logisticsPathRemark: [{required: true, message: "不能为空", trigger: "blur"}, {
          min: 5,
          max: 20,
          message: '长度在 5 到 20 个字符',
          trigger: 'blur'
